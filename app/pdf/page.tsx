const cases = [
  {
    ...
    result:
      "月約20時間の作業工数削減、月15時間超の残業解消、10施設共通の業務基盤を整備。",
  },
  {
    title: "購買BPRによるコスト削減",
    issue: "施設・部署ごとの個別発注により、過剰在庫や廃棄ロスが発生。",
    action: "在庫管理を一元化し、全施設規模での集中購買体制を構築。",
  ...
  return (
    <main className="bg-slate-200 py-8 text-slate-900 print:bg-white print:py-0">
      <div className="mx-auto max-w-[900px] space-y-6 print:max-w-none print:space-y-0">
        {/* Page 1 */}
        <section className="pdf-page overflow-hidden rounded-2xl bg-white shadow print:rounded-none print:shadow-none">
          <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 p-10 text-white">
            ...
            <h1 className="mt-5 text-5xl font-black leading-tight tracking-tight">
              米山徳彦
              <span className="mt-3 block text-2xl font-bold text-cyan-100">
                現場起点DXポートフォリオ
              </span>
            </h1>

            <p className="mt-8 max-w-2xl text-2xl font-bold leading-relaxed">
              公共福祉の現場から、業務改革と生成AI活用を実装する。
            </p>

            <p className="mt-6 max-w-3xl text-sm leading-7 text-slate-200">
          ...
          <div className="p-10">
            <div className="grid grid-cols-4 gap-3">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <p className="text-3xl font-black text-slate-950">約20年</p>
                <p className="mt-2 text-xs leading-5 text-slate-600">
                  公共福祉施設の運営・管理経験
                </p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <p className="text-3xl font-black text-slate-950">10施設</p>
                <p className="mt-2 text-xs leading-5 text-slate-600">
                  横断の業務標準化・BPR推進
                </p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <p className="text-3xl font-black text-slate-950">月20h</p>
                <p className="mt-2 text-xs leading-5 text-slate-600">
                  業務管理ツールによる工数削減
                </p>
              </div>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 p-6">
              <h2 className="text-xl font-black text-slate-950">提供できる価値</h2>
              <ul className="mt-4 grid grid-cols-2 gap-3 text-sm leading-6 text-slate-700">
                {strengths.map((item) => (
            ...
            <div className="mt-8 rounded-2xl bg-slate-950 p-6 text-white">
              <p className="text-sm font-bold tracking-[0.2em] text-cyan-200">
                WEB PORTFOLIO
              </p>
            ...
            {cases.slice(0, 2).map((item, index) => (
              ...
              >
                ...
                <div className="mt-5 space-y-4 text-sm leading-7">
                  <p>
                    <span className="font-bold">課題：</span>
                    {item.issue}
                  </p>
                  <p>
